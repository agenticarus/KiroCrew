...
class CronService:
    ...
    async def _force_reap(
        ...
        *,
        ...
        # refuses it. A kill that raised (``_sigkill_session`` on a refused
        # pid, the reaper task cancelled at shutdown) would otherwise leave the
        # claim taken for the life of the gateway -- the sweep swallows this
        # coroutine's exception and skips the claim on every later sweep, and
        # every manual run of the job 409s. The failure is carried into the
        # terminal record below (the run's finally writes none for a taken run)
        # and re-raised after it, so the sweep still logs it.
    ...
    async def cancel(self, job_id: str) -> bool:
        ...
        # discard_finished_run refuses it. A kill that raised (the executor
        # shut down under the hop, ``_sigkill_session`` on a refused pid, this
        # handler cancelled when the client disconnects) would otherwise leave
        # the claim taken until restart: every manual run of the job 409s and
        # every scheduled fire is skipped. The failure is carried into the
        # terminal record (the run's finally writes none for a taken run) and
        # re-raised after it, so the caller still learns the kill failed.
        killed_proc = False
        kill_failure: BaseException | None = None
        try:
