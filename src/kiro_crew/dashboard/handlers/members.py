...
async def _takeover_stopped_loop(
    ...
    resume itself the outcome is unknown -- the service shields its persist and
    may still commit -- so the entry is left as written: a stopped loop under
    an owner entry is resumable only by the owner's switch, and the store's
    remove or the owner's OFF revokes it. One entry per loop, one party per
    entry: see ``record_owner_arm``.
    """
