...
def _scope_has_entry(name: str, path: Path) -> bool:
    ...
    return isinstance(_read_scope_json(path)[0].get("mcpServers", {}).get(name), dict)
...
def _set_scope_entry(path: Path, name: str, *, enabled: bool, spec: dict | None = None) -> str:
    ...
    file would see ``{}`` and an apply would report ``noop``-into-destruction
    -- the panel sees the server via discovery's tolerant read, while the
    strict re-read sees an empty file and writes a skeleton-plus-entry over
    the user's whole config. Every write, though, is a plain-JSON rewrite of the entire
    document, so a file that parsed only tolerantly -- or not at all -- is
    refused via :class:`_McpConfigRefused` instead of written: no-op paths
    that touch nothing still succeed on such a file.
    """
            ...
            for change in changes:
                ...
                if change.get("uninstall"):
                    ...
                    try:
                        ...
                        # The user's global file holds JSONC (or worse) and the
