...
async def _fetch_usage_bg() -> str | None:
    ...
    async def _adjacent_identity() -> dict[str, object] | None:
        """Re-resolve whoami right before a decision about the cache is made.

        The identity read at the top of the refresh can be a minute or more old
        by the time a path decides what to publish or keep (the API attempt plus
        the scrape's own timeout), and a profile switch can land inside that
        window. Judging against the top-of-refresh value would then match the
        PREVIOUS account: a failure path would keep its balance on screen under
        the new one, and the API success path would publish its balance and
        email as the new one's. So every such decision judges against a whoami
        resolved adjacently, the same way the parseable-plan scrape branch does
        before it labels its reading. ``None`` when kiro-cli was never resolved:
        nothing can be proven, nothing is kept or published. whoami costs no
        credits and is bounded, so this adds at most one short subprocess.
        """
    ...
    async def _refresh() -> str | None:
        ...
        if parsed.get("credits_plan") is not None:
            ...
            # above follows the same contract with one more step: its numbers
            # come from a credential anchored on the TOP identity, so it first
            # requires its own adjacent whoami to PROVE that account is still
            # the one signed in (`_whoami_agreement`), then couples the identity
            # fields to the billed ARN (`_identity_matches_account`) against
            # that adjacent whoami. Both branches publish only what the account
            # signed in NOW can be shown to own.
            parsed.update(
