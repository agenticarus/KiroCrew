...
#: module docstring. Membership is NECESSARY and not sufficient: each of these two
#: must also clear its own confidence bar, and :func:`earns_badge` is the one
#: function that answers the whole question.
#:
#: The tuple has ONE consumer: it is the admission guard at the top of
#: :func:`earns_badge`, so a tier outside it is refused before any bar is read. It
#: is also what the frontend mirror (``toolRiskRecord.TOOL_RISK_TIERS``) copies.
#: :func:`earns_badge` then prices each admitted tier in its own arm instead of
#: looping this tuple, so each bar stays a module attribute a test can move ALONE;
#: a mapping built here would capture both values at import and make that
#: untestable. The cost of that choice is this sentence: a tier added to this tuple
#: needs its own arm in :func:`earns_badge`, or it is admitted, priced by no bar,
#: and refused -- never priced by a neighbour's bar.
...
def earns_badge(tier: str, p: float) -> bool:
    ...
    :data:`FLAGGED_TIERS` admits; the arms price. Each arm reads its bar at CALL
    time, so a test can move one bar and leave the other where it was. The last
    line is deliberate: a tier the tuple admits and no arm prices is refused, never
    handed a neighbour's bar.
    """
    if tier not in FLAGGED_TIERS:
        return False
    if tier == TIER_RISKY:
        return p >= RISKY_CONFIDENCE_THRESHOLD
    if tier == TIER_CAUTION:
        return p >= CAUTION_CONFIDENCE_THRESHOLD
    return False  # admitted by the tuple, priced by no bar -> refuse, never guess
