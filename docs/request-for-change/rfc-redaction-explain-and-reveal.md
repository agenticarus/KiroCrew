...
The rule for what may be kept is that **we keep only what the removers
themselves would pass.** It would be wrong to assume the query is the dangerous
part and the path is safe — a link can carry smuggled data in its path just as
easily — so every part we keep is put through the same checks that blocked the
link, and a part that fails those checks is not kept. When nothing passes, the
chip shows the site name and the reason, which is still more than today. There
is one precedent for keeping a URL this way: the consent link for a connected
server (`oauth_url`) is the single value preserved in a message's meta today,
and it is preserved only when it passes two checks. Anything kept is still
text an agent wrote, so the chip escapes it and never makes it clickable.

That splits the decision in two, and only the first half ships in step 3:

- **Structure only** — the site, the path when it passes, how long the query
  is, and why the link was blocked. This is enough for the chip and the
  details card as described above, and it carries none of the smuggled value.
- **The query value itself** — needed only for *Copy URL* and *Open once*.
  This keeps the payload in the saved conversation and puts sending it one
  confirmation away, so it is not part of step 3. If it ships later, *Open
  once* must re-run the checks at the moment of the click rather than trust
  what was saved, because a rule that changed in between must still apply.
