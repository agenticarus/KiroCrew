...
test("a shipped bundled kiro-cli is exported as its directory", () => {
  ...
  // what runs. A DIRECTORY, not the binary: kiro-cli exec-dispatches to
  // sibling executables, so the layout must resolve together.
  const seen = [];
  const fakeFs = {
    statSync(target) {
      seen.push(target);
