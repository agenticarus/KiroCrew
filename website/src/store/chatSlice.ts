  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(warmSlotCache.fulfilled, (state, action) => {
        ...
        // A coverage retry carries the bounded read's total because the wide
        // read's own total counts the raw uncollapsed corpus, running or idle,
        // and is not comparable with the collapsed counts later pages report.
        const comparable = (action.payload as { comparableTotal?: number }).comparableTotal
        retainServerTotal(
          state, key, comparable ?? total, running, warmSeq,
