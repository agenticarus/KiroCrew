...
describe('RestartButton', () => {
  ...
  it('surfaces the Error message on failure through ErrorNotice, and keeps it until dismissed', async () => {
    ...
    expect(notice).toHaveTextContent('zzq-restart-broke')
    // Off by default: the mount decides whether a hand-off can destroy a draft.
  ...
  it('offers the agent hand-off only when the mount opts in', async () => {
    restartSessions.mockRejectedValue(new Error('zzq-restart-broke'))
    render(<RestartButton askAgent />)

    fireEvent.click(screen.getByRole('button', { name: /apply & restart/i }))
    await screen.findByTestId('restart-button-error')
