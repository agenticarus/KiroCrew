  ...
  return (
    <div className="fixed bottom-safe-offset-4 right-safe-offset-4 z-40 max-w-sm flex flex-col gap-2" data-testid="meet-crewmates-eligibility-error">
      ...
      <button
        ...
        onClick={onDismiss}
        data-testid="meet-crewmates-eligibility-dismiss"
      >
        {t('app.dismiss')}
      </button>
    </div>
...
export default function MeetCrewmatesFlow({
  ...
  useEffect(() => {
    ...
    setSchedule('saved')
    setCreatedName('')
    // An unanswered create belongs to THIS opening: a later, independent
    // same-name crewmate must not inherit this flow's schedule on a 409.
      ...
      if (spec) {
        try {
          ...
          if (e instanceof ApiError && e.status >= 400 && e.status < 500) {
            ...
            // The job's `member_id` is the server's slug for the crewmate (lower
            // case, suffixed on collision), never the typed display name, so the
            // match goes through the roster row the create produced.
            const [listed, roster] = await Promise.all([api.crons().catch(() => null), api.members().catch(() => null)])
            const slug = roster?.members.find(m => m.name === crewmate)?.slug
            outcome = slug && listed?.jobs?.some(j => j.member_id === slug) ? 'saved' : 'unknown'
          }
        }
      }
        ...
        <div className="flex items-start gap-5">
          ...
          <div className="min-w-0 flex-1">
            ...
            {!nameError && nameShapeHint && (
              <p id="meet-crewmates-name-hint" className="mt-2 text-[12px] text-muted" data-testid="meet-crewmates-name-hint">
                {nameShapeHint}
              </p>
            )}
        ...
        <div className="mt-5">
          ...
          >
            ...
            <div className="flex items-center justify-between gap-4 px-4 py-3" data-testid="meet-crewmates-slack-row">
              ...
              {/* Plain text, no pill or border: chrome here read as a disabled
                  switch. The state word sits where the toggle above sits. */}
              <span className="shrink-0 text-[13px] text-muted" data-testid="meet-crewmates-slack-state">
                {slackReady
