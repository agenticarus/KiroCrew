...
@pytest.mark.asyncio
async def test_a_legacy_privacy_flag_refuses_without_opening_the_transcript_until_the_prune_records_it(
    tmp_path, monkeypatch, caplog
):
    ...
    copies the mode into the existing transcript's header and collects the row,
    after which a header-only reader refuses on its own.
    """
    import logging
    import threading
    from types import SimpleNamespace
    ...
    try:
        ...
        # row goes, and neither happened on the loop thread.
        ...
        assert sm.get_flag(live_key, "incognito") is False
        assert real_get_metadata(log, live_key).get("memory_mode") == "incognito"
        assert log.unconsolidated_count(live_key) == 3

        # From here the header answers alone: a consolidator with no map refuses
        # (the guard against opening the transcript comes off -- the header IS
        # what this reader is meant to open).
